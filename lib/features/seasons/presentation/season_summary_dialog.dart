...
class SeasonSummaryDialog extends StatelessWidget {
  ...
  String get _monthTitle {
    final name =
        DateFormat('LLLL yyyy', 'ru').format(DateTime(summary.year, summary.month));
    ...
    return AlertDialog(
      ...
      content: Column(
        ...
        children: [
          ...
          _row(theme, Icons.task_alt, 'Задач выполнено',
              '${summary.tasksCompleted}'),
          _row(theme, Icons.repeat, 'Привычек отмечено',
              '${summary.habitsCompleted}'),
          _row(theme, Icons.local_fire_department, 'Лучший стрик',
              '${summary.bestStreak}'),
          if (summary.topAxisName != null)
            _row(theme, Icons.hexagon, 'Топ-ось', summary.topAxisName!),
          const Divider(height: 20),
          _row(theme, Icons.diamond, 'Бонус кристаллов',
              '+${summary.gemsAwarded}'),
          _row(theme, Icons.military_tech, 'Престиж',
              '${summary.newPrestige}'),
